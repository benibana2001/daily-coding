...
class Board {
    ...
    static putWhite = (board: BoardState) => (s: Square): BoardState => putToken(board)(s)(Token.WHITE)
    static putBlack = (board: BoardState) => (s: Square): BoardState => putToken(board)(s)(Token.BLACK)
    ...
    static scanLinePattern = (board: BoardState) => (d: DIRECTION) => (center: Square): scanedLineData => {
        ...
        for (let n = 1; ; n++) {
            ...
            scanedData.pattern += convertTokenToStrings(getToken(board, currentSquare))
    ...
    private initBoard = (imgItems: imgItem[] | null = null) => {
        ...
        (async () => { 
            ...
            this.state.canPutSquares.forEach((square: Square) => this.putSquareColor(square)(this.resources.getimg('voidGreen')));
        })();
    }
    ...
    private drawBoard = async (imgItems: imgItem[]) => {
        ...
        await this.createVoidImages(this.squareSize, 'voidGreen', 'rgba(0, 255, 0, 0.16)', 1)

        Board.walk((s: Square) => this.drawTokenImage(s))
    }

    private drawTokenImage = (square: Square) => {
        drawToken(this.ctx, this.board, { white: this.resources.getimg('white'), black: this.resources.getimg('black') }, this.squareSize, square)
    }
    ...
    private putToken = (square: Square) => putToken(this.board)(square)(this.state.player)

    public updatePutSquare = (clickedSquare: Square) => {
        this.state.canPutSquares.forEach((square: Square) => this.putSquareColor(square)(this.resources.getimg('voidWhite')))
    ...
    public updateReverseSquares = (clickedSquare: Square) => {
        ...
        reverseSquares.forEach((clickedSquare: Square) => {
            this.putToken(clickedSquare)
            this.ctx.drawImage(this.resources.getimg('voidWhite'), this.squareSize * clickedSquare.col, this.squareSize * clickedSquare.row)
            this.drawTokenImage(clickedSquare)
        })
    }
    ...
    public createVoidImages = async (size: number, name: string, color: string, offset = 0) => {
...
export const getToken = (board: BoardState, s: Square): Token => board[s.row][s.col]
export const putToken = (board: BoardState) => (s: Square) => (token: Token): BoardState => {
    board[s.row][s.col] = token
    return board
}

const outOfBoard = (board: BoardState) => (s: Square) => {
    return board[s.row] === undefined
        || getToken(board, s) === undefined
}

// Draw
const drawLine = (ctx: CanvasRenderingContext2D, row: number, col: number, squareSize: number): void => {
    for (let i = 0; i < col; i++) {
        for (let j = 0; j < col; j++) {
