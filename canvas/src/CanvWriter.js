export default class Canv {
  ...
  static frameCalc = (
    ...
    head,
    reverse = false
  ) => (tick) => frameCalc(framesData, frameLength, speed, head, reverse)(tick)
...
function fitBackgroundScale(context, imgOriginalWidth, maxScale) {
  ...
  const x = cw / imgOriginalWidth <= maxScale ? cw / imgOriginalWidth : maxScale
  const y = x
  context.scale(x, y)
  return [x, y]
}
...
const frameCalc = (framesData, frameLength, speed, head, reverse = false) => (
  tick
) => {
